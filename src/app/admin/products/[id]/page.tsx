  ...
  const handleSave = async () => {
    ...
    try {
      const productData = {
        ...
        type,
        price: type === 'simple' ? price : (variants.length > 0 ? Math.min(...variants.map(v => v.price)) : 0),
        sale_price: type === 'simple' && salePrice ? Number(salePrice) : (type === 'variable' && variants.some(v => v.sale_price) ? Math.min(...variants.filter(v => v.sale_price).map(v => v.sale_price!)) : null),
        stock: type === 'simple' ? stock : variants.reduce((sum, v) => sum + v.stock, 0),
